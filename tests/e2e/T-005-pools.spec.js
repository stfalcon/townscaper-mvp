...
test.describe('T-005: InstancedMesh pools render cells (buildings + land)', () => {
  test('single land cell appears in scene via state.setCell', async ({ page }) => {
    await page.goto('/');
    ...
    const result = await page.evaluate(() => {
      ...
      state.setCell(15, 0, 15, { type: 'land' });
      return renderer.getPoolStats();
    });

    expect(result.land).toBe(1);
  ...
  test('migration between pools when tileType changes', async ({ page }) => {
    ...
    await page.evaluate(() => window.__game__.state.setCell(10, 0, 10, { type: 'land' }));
    await sleep(50);
    ...
    expect(base).toMatchObject({ total: 1, land: 1, roof: 0 });
    expect(stack).toMatchObject({ total: 2, land: 1, roof: 1 });
    expect(unstack).toMatchObject({ total: 1, land: 1, roof: 0 });
  });
